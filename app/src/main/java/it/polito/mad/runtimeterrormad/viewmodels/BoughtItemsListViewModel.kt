...
class BoughtItemsListViewModel(private val userID: String) : ViewModel() {
    ...
    private fun loadItems(): MutableLiveData<List<Item>> {
        ...
        _itemsListener =
            ItemRepository.getUserBoughtItems(userID).addSnapshotListener { snapshot, e ->
                when {
                    e != null -> Log.e(
                        Constants.BOUGHT_ITEMS_LIST_VIEWMODEL,
                        "Error with Item=$userID"
                    )
                    snapshot != null -> {
                        ...
                        for (doc in snapshot) {
                            ...
                            listTmp.add(itemTmp)
                            Log.d(Constants.BOUGHT_ITEMS_LIST_VIEWMODEL, "Item Update=$itemTmp")
                        }
                        mutable.value = listTmp
                    }
                    else -> Log.d(
                        Constants.BOUGHT_ITEMS_LIST_VIEWMODEL,
                        "Null snapshot with Item=$userID"
                    )
                }
            }
        return mutable
